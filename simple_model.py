...
safety_stock = {i: rnd.random() for i in products}
...
inventory_balance = inventoryModel.addConstrs((inventory_level[product, time_periods[i-1]] + replenishment_q[product, time_periods[i]] == demand_forecast[(product, time_periods[i])] + inventory_level[product, time_periods[i]] for product in products for i in range(1,time_periods+1)), name="InventoryBalance")
...
minimum_inventory = inventoryModel.addConstrs((inventory_level[product, time_period] >= (1- gp.quicksum(order_product[product, time_period, tau_period] for tau_period in tau_periods[:len(time_periods)-time_period])) * safety_stock[product, time_period, "tau1"] + gp.quicksum(order_product[product, time_period, tau_period]*(safety_stock[product, time_period, tau_period] + gp.quicksum(demand_forecast[(product, time_period, 1+x)] for x in range(1,tau_period))) for tau_period in tau_periods[:len(time_periods)-time_period]) for product in products for time_period in time_periods), name="minimumInventory")

#objective function
obj = gp.quicksum(major_setup_cost*place_order[time_period] for time_period in time_periods) + gp.quicksum(minor_setup_cost * gp.quicksum(order_product[product, time_period, tau_period] for tau_period in tau_periods[:len(time_periods)-time_period]) for product in products for time_period in time_periods) + gp.quicksum(holding_cost[product]*inventory_level[product, time_period] for product in products for time_period in time_periods)
